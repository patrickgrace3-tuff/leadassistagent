...
async function load() {
  ...
  // Token: show only a value saved here, not one coming from config.local.js,
  // so a blank field clearly means "using config.local.js (or none)".
...
async function save() {
  ...
  // Blank token here → fall back to config.local.js rather than overriding it.
